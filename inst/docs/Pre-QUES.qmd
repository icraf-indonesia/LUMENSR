...
# Similarly, load the land cover data for period 2020, convert it to a raster, and add the legend
t2 <- 2020
...
# Load the planning unit data and convert it to a raster
# Note: 'ntt_admin' must be defined somewhere earlier in the code
admin_z <- ntt_admin %>% rasterise_multipolygon()

# Define the area cutoffs for landscape and planning unit level analysis
# These cutoffs determine the minimum area required for a patch to be considered in the analysis
cutoff_landscape <- 5000
cutoff_pu <- 500

# Extract the time information from the land cover rasters
...
# and summarization and return a list of results. The 'convert_to_Ha' argument specifies whether to convert
# the area measurements to hectares.
ques_pre_output <- ques_pre(lc_t1_, lc_t2_, admin_z, 
                            cutoff_landscape = cutoff_landscape, 
                            cutoff_pu = cutoff_pu, 
                            convert_to_Ha = TRUE)
```
